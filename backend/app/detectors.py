...
class NextJSDetector:
    ...
    def detect_routes(self, file_path: Path, content: str) -> DetectorResult:
        ...
        try:
            ...
            if routes:
                record_detector_hit("nextjs_routes")
...
class ExpressDetector:
    ...
    def detect_routes(self, file_path: Path, content: str) -> DetectorResult:
        ...
        try:
            ...
            if routes:
                record_detector_hit("express_routes")
                return DetectorResult(
                    items=routes,
...
class ReactRouterDetector:
    ...
    def detect_routes(self, file_path: Path, content: str) -> DetectorResult:
        ...
        try:
            ...
            if routes:
                record_detector_hit("react_router")
                return DetectorResult(
                    items=routes,
                    confidence=0.8,
                    hypothesis=False,
...
class QueueDetector:
    ...
    def detect_jobs(self, file_path: Path, content: str) -> DetectorResult:
        ...
        try:
            ...
            if jobs:
                record_detector_hit("queue_jobs")
                return DetectorResult(
                    items=jobs,
                    confidence=0.8,
                    hypothesis=False,
...
class StoreDetector:
    ...
    def detect_stores(self, file_path: Path, content: str) -> DetectorResult:
        ...
        try:
            ...
            if stores:
                record_detector_hit("data_stores")
                return DetectorResult(
                    items=stores,
                    confidence=0.8,
                    hypothesis=False,
...
class ExternalDetector:
    ...
    def detect_externals(self, file_path: Path, content: str) -> DetectorResult:
        ...
        try:
            ...
            if externals:
                record_detector_hit("external_services")
                return DetectorResult(
                    items=externals,
                    confidence=0.8,
                    hypothesis=False,
